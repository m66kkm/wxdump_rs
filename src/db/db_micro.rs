...
impl MicroHandler {
    ...
    /// Get contact list
    pub fn get_contact_list(&self, limit: usize, offset: usize) -> WxCoreResult<Vec<serde_json::Value>> {
        wx_core_error(|| {
            let sql = format!(
    ...
    pub fn get_contact_by_username(&self, username: &str) -> WxCoreResult<Option<serde_json::Value>> {
        wx_core_error(|| {
    ...
    /// Search contacts
    pub fn search_contacts(&self, keyword: &str, limit: usize, offset: usize) -> WxCoreResult<Vec<serde_json::Value>> {
        wx_core_error(|| {
            let sql = format!(
                "SELECT * FROM contact 
        ...
        wx_core_error(|| {
            ...
            if let Some(serde_json::Value::Object(map)) = result {
                if let Some(serde_json::Value::String(members)) = map.get("memberlist") {
                    ...
                    let members_vec = members.split(';').map(|m| {
                        serde_json::json!({
                            "username": m.trim(),
                        })
                    }).collect();
