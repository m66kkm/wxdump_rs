...
impl DBHandler {
    ...
    pub fn new(db_path: impl AsRef<Path>) -> WxCoreResult<Self> {
        wx_core_error(|| {
            ...
            if !db_path.exists() {
                return Err(WxCoreError::InvalidPath(format!("Database file not found: {}", db_path.display())));
            }
    ...
    pub fn execute_query(&self, sql: &str, params: &[&dyn rusqlite::ToSql]) -> WxCoreResult<Vec<serde_json::Value>> {
        wx_core_error(|| {
    ...
    pub fn execute_query_one(&self, sql: &str, params: &[&dyn rusqlite::ToSql]) -> WxCoreResult<Option<serde_json::Value>> {
        wx_core_error(|| {
