        ...
        if is_print {
            ...
            for (i, info) in result.iter().enumerate() {
                ...
                println!(
                    "[+] {:>8}: {}",
                    "account",
                    info.account.as_deref().unwrap_or("None")
                );
                println!(
                    "[+] {:>8}: {}",
                    "mobile",
                    info.mobile.as_deref().unwrap_or("None")
                );
                println!(
                    "[+] {:>8}: {}",
                    "nickname",
                    info.nickname.as_deref().unwrap_or("None")
                );
                println!(
                    "[+] {:>8}: {}",
                    "mail",
                    info.mail.as_deref().unwrap_or("None")
                );
                println!(
                    "[+] {:>8}: {}",
                    "wxid",
                    info.wxid.as_deref().unwrap_or("None")
                );
                println!(
                    "[+] {:>8}: {}",
                    "key",
                    info.key.as_deref().unwrap_or("None")
                );
                println!(
                    "[+] {:>8}: {}",
                    "wx_dir",
                    info.wx_dir.as_deref().unwrap_or("None")
                );
        ...
        if let Some(path) = save_path {
            ...
            if path.exists() {
                match File::open(&path) {
                    Ok(file) => match serde_json::from_reader::<_, Vec<WxInfo>>(file) {
...
pub fn get_wx_db(
    ...
    wx_core_error(|| {
        ...
        let msg_dir = match msg_dir {
            ...
            _ => {
                ...
                match get_wx_dir_by_reg(None) {
                    Some(dir) => dir,
                    None => {
                        return Err(WxCoreError::InvalidPath("无法获取微信文件目录".to_string()))
                    }
                }
            }
        };
        ...
        // Parse wxids
        let wxids: Option<Vec<String>> =
            wxids.map(|s| s.split(';').map(|s| s.to_string()).collect());

        // Parse db_types
        let db_types: Option<Vec<String>> =
            db_types.map(|s| s.split(';').map(|s| s.to_string()).collect());
...
pub fn get_core_db(wx_path: &Path, db_types: Option<Vec<&str>>) -> WxCoreResult<Vec<WxDbInfo>> {
    wx_core_error(|| {
        ...
        let db_types = match db_types {
            Some(types) => types
                .iter()
                .filter(|&&t| CORE_DB_TYPE.contains(&t))
                .copied()
                .collect(),
