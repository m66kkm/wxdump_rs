...
pub fn decrypt(
    ...
    wx_core_error(|| {
        ...
        let password = hex::decode(key.trim())
            .map_err(|_| WxCoreError::Key(format!("key: '{}' Invalid hex!", key)))?;
        ...
        if db_path.is_file() {
            ...
            for entry in walkdir::WalkDir::new(db_path) {
                ...
                if entry.file_type().is_file() {
                    ...
                    let out_file =
                        format!("de_{}", rel_path.file_name().unwrap().to_string_lossy());
            ...
            return Err(WxCoreError::InvalidPath(format!(
                "db_path: '{}' is neither a file nor a directory!",
                db_path.display()
            )));
        }
